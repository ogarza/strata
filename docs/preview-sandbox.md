...
Thumbnail rendering uses a four-slot scheduler, limits RAW/PDF/video heavy work to one of those slots, and queues at most 64 unique requests. Raster image misses use a lazy persistent worker pool after the parent copies the source into a sealed bounded snapshot memfd. RAW, PDF, video thumbnails, and previews remain on their existing one-shot helper paths. Live rows deferred by a full queue are retried as capacity opens, duplicate requests share one render, rows leaving the view cancel queued work that has no remaining targets, and failed renders are cached for 30 seconds to prevent retry loops.
...
The protocol module introduced for D06a defines, documents, and tests the persistent thumbnail worker control plane. D06b proved one reusable raster worker, and D07 routes production raster-image misses through a lazy supervised pool using the same sealed-snapshot protocol. Production RAW/PDF/video thumbnails and previews still use one-shot helpers.
...
The raster pool starts with zero helpers and spawns only for admitted image misses. Disk-cache hits do not create a sandbox. Startup and each request/reply are supervised by the worker owner thread with bounded protocol deadlines. Decode failures are bounded job failures and leave the helper reusable; crash, timeout, protocol failure, failed startup, or shutdown retire the helper. Helpers idle out after 30 seconds. Repeated startup failure enters process-wide backoff so missing bubblewrap or namespace support cannot create a spawn storm. Before an incompatible one-shot thumbnail backend starts, one idle raster helper is retired so the migration period remains within the same total render budget.
